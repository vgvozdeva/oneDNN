...
public:
    ...
    bool is_compatible(tensor_kind_t abc, const v2::layout_t &layout) const {
    ...
    v2::layout_t to_compatible_layout(
            tensor_kind_t abc, const v2::layout_t &layout) const {
        auto ret = layout;
        switch (abc) {
    ...
    v2::layout_t acc_layout(const v2::layout_t &a_layout,
            const v2::layout_t &b_layout, const v2::layout_t &c_layout) const {
        gpu_assert(a_layout.has_const_sizes());
        gpu_assert(b_layout.has_const_sizes());
        v2::layout_t acc(c_layout.desc(), acc_type());
    ...
    v2::layout_t bias_layout(const v2::layout_t &b_layout,
            const v2::layout_t &bias_layout) const {
        gpu_assert(b_layout.has_const_sizes());
        v2::layout_t acc(bias_layout.desc(), acc_type());
...
private:
    ...
    bool init_mad(const layout_desc_t &a_desc, const layout_desc_t &b_desc,
        ...
        for (auto &d : iter_tile_) {
            ...
            if (is_n(d) || is_b(d)) {
                ...
                b_inner_ = v2::layout_t(b_desc, b_type_, 0, {std::move(block)});
                break;
            }
        }
        gpu_check(found) << "init_mad: cannot find dimension to vectorize.";
        c_inner_ = v2::layout_t(c_desc, acc_type_, 0, b_inner_.blocks());
        return true;
    }
    ...
    static v2::layout_t to_v2_layout(const jit::layout_t &layout,
...
private:
    static send_plan_t try_create_send_plan(const std::string &tag,
            const send_params_t &params, const v2::view_t &view) {
    ...
    static bool check_compatible_layout(
            const v2::layout_t &layout, const tile_t &tile) {
        for (auto &d : tile) {
    ...
    bool init_x_prefetch_plan(tensor_kind_t abc, const coord_t &coord,
        ...
        v2::grid_splitter_t grid_splitter;
        for (auto &d : thr_grid_.all_dims()) {
            grid_splitter.add(
                    thr_grid_.index_var(d), desc_.thread_group_tile[d]);
        }
        auto view
                = v2::view_t::split(mapper, layout, coord, tile, grid_splitter);
    ...
    bool init_fma_plan(const v2::layout_t &a, const v2::layout_t &b,
    ...
    bool init_x2r_fma_plan(x2r_fma_plan_t &plan, prb_reqs_t &reqs) const {
        ...
        v2::layout_t a_prev_layout;
        v2::layout_t b_prev_layout;
        v2::layout_t c_prev_layout;
        v2::layout_t bias_prev_layout;
        int c_off_elems = 0;
        ...
        for (int i = 0; i < outer_size; i++) {
            ...
            if (is_outer_m || i == 0) {
                auto a_sub_view
                        = v2::view_t(a_mapper, a_layout_, sub_coord, sub_tile);
                x2r_plan_t a;
            ...
            if (!is_outer_m || i == 0) {
                auto b_sub_view
                        = v2::view_t(b_mapper, b_layout_, sub_coord, sub_tile);
                x2r_plan_t b;
    ...
    bool init_epilogue_store_bias(bool is_atomic,
            const v2::layout_t &bias_reg_layout,
    ...
    bool init_slm_reduce_plan(const v2::layout_t &c_layout,
            virt_grid_t &virt_grid, slm_reduce_plan_t &plan) const {
        pvar_t k_dim;
        ...
        v2::layout_t slm_layout(mapper.layout_desc(), c_layout.type(),
        ...
        auto split_view = v2::view_t::split(
    ...
    bool init_epilogue_store_plan(bool is_atomic,
    ...
    bool init_epilogue_plan(const v2::layout_t &c_fma_layout,
        ...
        auto c_mem_view = v2::view_t(c_mapper, c_layout_, c_coord, c_tile);
    ...
    std::vector<pvar_t> skip_mask(const v2::view_t &view) const {
        return v2::skip_mask(view, coord_info_.tg_iter_tile(), reqs_);
    }
    ...
    v2::layout_t a_layout_;
    v2::layout_t b_layout_;
    v2::layout_t c_layout_;
    v2::layout_t bias_layout_;
    prb_reqs_t reqs_;
};
